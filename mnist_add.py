    ...
    def train(self, X, y, epochs, batch_size=8):
        ...
        for _ in range(epochs):
            for i in range(0, n, batch_size):
                ...
                delta = subtract_matrix(acts[-1], y_batch)
...
def train_mnist_model():
    X = load_mnist_images("train-images.idx3-ubyte", limit=2000)
    y_lbl = load_mnist_labels("train-labels.idx1-ubyte", limit=2000)
    y = one_hot(y_lbl, 10)

    net = FlexibleANN([784, 64, 32, 10], lr=0.05, momentum=0.9)
    net.train(X, y, epochs=100, batch_size=8)
    return net

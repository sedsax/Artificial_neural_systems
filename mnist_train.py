...
def train_mnist_model():
    ...
    y_train = one_hot(y_train_lbl, 10)

    X_test = load_mnist_images("t10k-images-idx3-ubyte", limit=500)
    y_test_lbl = load_mnist_labels("t10k-labels-idx1-ubyte", limit=500)

    # ---- TRAIN ----
    net = FlexibleANN([784, 64, 32, 10], lr=0.05, momentum=0.9)
    net.train(X_train, y_train, epochs=10, batch_size=16)
...
class MNISTDrawWindow:
    ...
    def predict(self):
        ...
        self.label.config(text=f"Prediction: {pred}  (conf={round(max(probs),2)})")
